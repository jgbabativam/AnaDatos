...
# PREVIOS {background-color="#0077b6"}
...
* Introducción a R: [Ver video](https://youtu.be/TlOKN0UzbB0)
* Importar datos: [Ver video](https://youtu.be/FkpIpLgI7es)
* Organización y transformación de datos. Videos:
...
1. Importe los datos, explore las variables y cantidad de registros
2. ¿Cuántos casos están con estado de activo?
3. ¿Cuál es el departamento con el mayor número de víctimas en los últimos 5 años?
4. ¿Es correcto comparar los departamentos según el número de víctimas?
5. ¿Cuántas víctimas se han registrado por año?, compare por sexo y por edad.
6. Haga un análisis con enfoques diferenciales según las variables de LGBTI, NNA, indígena y Afrodescendiente.

## Resultados

1. Cree el proyecto de trabajo
2. Descargue los datos de [acá](https://github.com/jgbabativam/AnaDatos/blob/main/datos/violencia_intrafamiliar.parquet)
...
p_load(tidyverse, arrow, scales)

url <- "https://raw.githubusercontent.com/jgbabativam/AnaDatos/main/datos/violencia_intrafamiliar.parquet"
fgn <- read_parquet(url)
...
ggplot(res, aes(x = ESTADO, y = cantidad)) +
...
ggplot(res, aes(x = ESTADO, y = cantidad)) +
  geom_col(fill = "#FFD700") +
  geom_text(aes(label = comma(cantidad)), vjust = -0.3) +
  theme_bw() +
  labs(y = "Cantidad")
```
...
## Resultados: Departamento con más víctimas
...
res <- fgn |> 
       filter(AÑO_HECHOS >= 2020) |> 
       mutate(DEPARTAMENTO_HECHO = case_when(
         grepl("San Andr", DEPARTAMENTO_HECHO) ~ "San Andrés y Providencia", 
         grepl("BOGOTÁ, D. C.", DEPARTAMENTO_HECHO) ~ "Bogotá D.C.",
         grepl("Boyaca", DEPARTAMENTO_HECHO) ~ "Boyacá",
         TRUE ~ DEPARTAMENTO_HECHO)) |> 
       group_by(DEPARTAMENTO_HECHO) |> 
       summarise(victimas = sum(TOTAL_VÍCTIMAS)) |> 
...
ggplot(res, aes(x = reorder(as.factor(DEPARTAMENTO_HECHO), victimas),
                y = victimas)) +
  geom_col(fill = "#FF8B55") +
  geom_text(aes(label = comma(victimas)), hjust = -0.1, size = 3) +
  theme_minimal() +
  coord_flip() +
  labs(y = "Número de víctimas",
       x = "") +
...
¿Es correcto comparar los departamentos según el número de víctimas?

```{r}
library(countdown)
countdown(minutes = 6, seconds = 00,  right = 0)
```
...
# shapefile de departamentos de Colombia
dep <- geodata::gadm(country = "COL", level = 1, path = tempdir()) |> 
       st_as_sf()

map_data <- dep |>
            left_join(res, by = c("NAME_1" = "DEPARTAMENTO_HECHO"))

ggplot(map_data) +
  geom_sf(aes(fill = victimas), color = NA) +
  scale_fill_gradient(
    low  = "#FFE5E5",
    high = "#8B0000",
    na.value = "grey90",
  ...
  theme_void() +
  labs(title = "Víctimas de violencia de género en Colombia (últimos 5 años)")
```
...
<span style="color:#FF8D21"> **Tu turno**:</span> Sobre el conjunto de datos de la Fiscalía, apoyese en la IA para obtener un gráfico de líneas, usando `ggplot2`, con la cantidad de víctimas por año. Ajustelo a su gusto.
...
<span style="color:#FF8D21"> **Tu turno**:</span> Sobre el conjunto de datos Fiscalía, apoyese en la IA para obtener un diagrama de cajas elegante que permita comparar los departamentos por la variable del total de víctimas en los últimos 5 años, use `ggplot2`. Ajustelo a su gusto.
...
Construya un plan de análisis y apóyese en la IA para llevarlo a cabo. Revise datos faltantes, distribuciones, outliers, asociación, etc.
...
# REPRODUCIBLES (Ver Video) {background-color="#0077b6"}
